...
- `.chezmoiscripts/run_onchange_after_100_vim.sh.tmpl` — vim plugin setup. **It feeds vim a FIFO on fd 9 rather than inheriting stdin, and that is load-bearing.** `vim -es` reads Ex commands from stdin, and vim-plug's `--sync` wait loop (`while … sleep 100m`) polls it between clones; an stdin at EOF ends the Ex session mid-install, killing the in-flight git jobs, skipping the remaining `-c` commands and exiting **0**. That is the stdin chezmoi supplies whenever it is not attached to a terminal — CI and the `curl | bash` bootstrap — so this installed nothing there while `set -e` reported success, and worked by hand only because an interactive apply inherits the tty. The `E31: No such mapping` the old comment blamed is a red herring: `s:prepare` does `silent! unmap <buffer> <cr>`, which sets `v:errmsg` without raising. Because vim exits 0 either way, the script then re-asks a fresh vim which entries of `g:plugs` have no directory and fails loudly on any that are missing — do not drop that second invocation for being redundant, it is the only thing that can observe the failure.
...
`git-delete-local-merged` is the only one of those with real logic, and it depends on a fact about **this GitHub repo**: `allow_squash_merge` and `allow_merge_commit` are off, so every PR lands as a **rebase merge**. That replays a branch onto `master` under new SHAs, which means the branch tip is never an ancestor of anything and `git branch --merged` — the predicate the script used for years — can never report it. It reported `No merged branches to delete.` on a repo full of merged branches.

So it takes a second route: every commit patch-identical to something upstream (`git cherry`) **and** no remote-tracking branch. Both, deliberately. Patch identity alone keeps a merged branch you are still using; an absent remote alone would delete a branch you abandoned and cleaned up without merging. git considers such branches unmerged, so this path deletes with `git branch -D` — which is why `-n`/`--dry-run` exists, and why naming branches explicitly is supported. If the repo's merge strategy ever changes, revisit the predicate rather than assuming the ancestry check carries it.

The second half of that predicate is **not** `%(upstream:track)` = `[gone]`, though it was at first, and that kept the script inert for the common case. `[gone]` requires a *configured upstream*, which only `git push -u` (or a bare `git push` under `push.autoSetupRemote`) creates. Pushing by name — `git push origin <branch>`, an explicit refspec, which is what `gh pr create` and most muscle memory do — sets none. Such a branch gets merged and its remote copy pruned by `fetch.prune` without git ever writing anything to the local ref, so it read as *"merged upstream, but its remote branch still exists"* and was silently kept forever. It now falls back to asking whether `refs/remotes/*/<branch>` exists at all, which covers both how a branch was pushed.
...
Note: `dot_config/zed/settings.json` (managed by chezmoi) is Zed's editor config, applied to `~/.config/zed/settings.json`; the Zed CLI symlink is handled by `.chezmoiscripts/darwin/run_after_20_zed_symlink.sh.tmpl`. `dot_config/ghostty/config.tmpl` (managed by chezmoi) is Ghostty's terminal config, applied to `~/.config/ghostty/config`. `dot_config/herdr/config.toml` (managed by chezmoi) is herdr's config, applied to `~/.config/herdr/config.toml`. `dot_config/glow/glow.yml` (managed by chezmoi) is glow's config, applied to `~/.config/glow/glow.yml`. `dot_config/nvim/` (managed by chezmoi) is Neovim's config, applied to `~/.config/nvim/` — see below.
...
**nvim is now what every editor hook points at**, which was not true when this config landed — it was introduced deliberately dormant, reachable only by typing `nvim`. The five places that named vim and now name nvim: `VISUAL`/`EDITOR` in `dot_zshenv`, `core.editor` in `dot_gitconfig.tmpl`, `v`/`ev` in `system/alii.zsh`, `egc`/`egi` in `git/git-alii.zsh`, and the `${EDITOR:-…}` fallback in `bin/secret`. vim is still installed and still fully configured, so `vim` at a prompt behaves exactly as it always did; nothing *defaults* to it any more. `invariants.bats` asserts that no editor hook has drifted back — the failure it guards against is silent, since a stale `vim` works fine and simply gives you the wrong editor.

`ev` changed target as well as editor: it was `vim ~/.vimrc`, it is now `nvim ~/.config/nvim`, because nvim never reads `~/.vimrc`. That is the *applied* config, so edits there drift from the chezmoi source — `erc` (Zed on `$DOTFILES`) is the tracked route. `~/.vimrc` has no alias any more; type `vim ~/.vimrc`.
...
**`lazy-lock.json` is deliberately not tracked.** lazy.nvim rewrites it on every `:Lazy update`, so a tracked copy would put `chezmoi status` permanently out of sync. `.chezmoiignore` carries an entry for it, but that entry is belt-and-braces — it is the only line in that file naming a *target* path rather than a source-dir one, and chezmoi does not document whether `chezmoi add` honours the file at all. What actually keeps the lockfile untracked is that it is never added, which is what `invariants.bats` asserts. The tradeoff accepted here: plugin versions are not reproducible across machines.

`neovim` is in the `$brews` list and in the linux installs `packages=()`. Kali is fine (it tracks Debian testing/unstable, currently 0.12.x, above LazyVim's 0.11.2 floor); Debian *stable* is not (trixie ships 0.10.4, bookworm 0.7.2), where nvim installs but LazyVim refuses to start with a version error.
...
**`width: 0` is load-bearing, and it does not mean "no wrapping".** glamour never reflows a paragraph — a soft line break in the source survives verbatim into the output, and `width` only re-wraps *individual source lines* that exceed it. So a width narrower than the file's own hard wrap does not re-fill the paragraph; it sheds the last word or two of every line onto a line of its own, giving a ragged column of one-word lines. This was `width: 100` — exactly what most Markdown is hard-wrapped at, and glamour's margins cost 4 columns, leaving 96 usable, so every line overflowed by a word. `0` reaches glow's detection branch (`main.go`: `if !cmd.Flags().Changed("width")` → terminal width, capped at 120, else 80 when stdout is not a TTY), which is wider than a 100-column source and still fits tables to the terminal. Passing `-w 0` on the *command line* is a different thing entirely: the flag counts as changed, wrapping is genuinely disabled, and a wide table runs straight past the terminal edge.

**`width: 0` narrows that problem without removing it.** The render field is `width - 4` and detection is capped at **120**, so a file hard-wrapped at 103 still stubs on a 104-column terminal, and one wrapped past ~116 stubs at any terminal size. glow has no reflow option — `--preserve-new-lines` does not affect softbreaks in either direction, verified against 3.0.0. The only durable fix is upstream of the renderer: do not hard-wrap paragraphs when writing Markdown in the first place.
