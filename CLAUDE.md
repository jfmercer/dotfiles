...
### Neovim / LazyVim (`dot_config/nvim/`) — separate from vim, deliberately
...
**Nothing here is shared with vim, and no vim settings were ported.** `dot_vimrc`, the `.vim/autoload/plug.vim` external and `run_onchange_after_100_vim.sh.tmpl` are untouched, `VISUAL`/`EDITOR` are still `vim`, and `system/alii.zsh`'s `v`/`ev` still open vim. The two editors need no guard to stay apart: Neovim does not read `~/.vimrc` and `~/.vim` is not on its `runtimepath` (verified both directions). There is no `nvim` alias because none is needed — `nvim` is the binary's own name.
